...
> Relatório no portal Assistência que consolida atendimentos com suporte a 29 filtros combinados e exportação em múltiplos formatos.
...
- Tabela de resultados (**DataTable**) com colunas padrão: Data, Protocolo, CNPJ Cliente, Plano, Tipo de Serviço, Beneficiário, CPF/CNPJ Beneficiário, Observação, Tipo de evento, Segmentação
- Exportação em .XLS, .CSV, .XLS colunas visíveis, .PDF
- Configuração de colunas persistível por usuário

---

## Regras de Negócio

- Campo **Período** preenchido automaticamente com os últimos **30 dias** como valor padrão
- Filtros combináveis em AND lógico — retorna apenas registros que atendem a todos os critérios
- Estado vazio exibe `"Nenhum resultado encontrado!"` na tabela
- Botão de limpar filtros restaura o formulário ao estado padrão (mantém Período = últimos 30 dias)
- Acesso sem autenticação redireciona para `/assistencia/login/` (RN-GLOBAL-001)
- Botões de exportação **desabilitados** antes da pesquisa e **habilitados** após resultados
- Tags: lógica AND (múltiplas = deve ter todas)
- Protocolo ignora período (busca direta)
